...
$CC -std=$CXX -O3 src/main.cpp -ldl -o build/dinfio
...
if [ "$COMPILE_MODULES" = true ]; then
    ...
    $CC -std=$CXX -O3 -fPIC -shared src/modules/fileio/fileio.cpp -o build/modules/fileio/fileio.so

    echo "Compiling module json..."
    $CC -std=$CXX -O3 -fPIC -shared src/modules/json/json.cpp -o build/modules/json/json.so

    echo "Compiling module math..."
    $CC -std=$CXX -O3 -fPIC -shared src/modules/math/math.cpp -o build/modules/math/math.so

    echo "Compiling module regex..."
    $CC -std=$CXX -O3 -fPIC -shared src/modules/regex/regex.cpp -o build/modules/regex/regex.so

    echo "Compiling module rl..."
    $CC -std=$CXX -O3 -fPIC -shared src/modules/rl/rl.cpp -o build/modules/rl/rl.so

    echo "Compiling module string..."
    $CC -std=$CXX -O3 -fPIC -shared src/modules/string/string.cpp -o build/modules/string/string.so

    echo "Compiling module time..."
    $CC -std=$CXX -O3 -fPIC -shared src/modules/time/time.cpp -o build/modules/time/time.so

    if [ "$COMPILE_URL_MODULE" = true ]; then
        echo "Compiling module url..."
        $CC -std=$CXX -O3 -fPIC -shared src/modules/url/url.cpp -o build/modules/url/url.so -lcurl
        cp -f src/modules/url/*.fio build/modules/url/
    fi

    if [ "$COMPILE_GUI_MODULE" = true ]; then
        echo "Compiling module gui..."
        $CC -std=$CXX -O3 -fPIC -shared -D__RPI__ src/modules/gui/gui.cpp -o build/modules/gui/gui.so `wx-config --cxxflags --libs core`
