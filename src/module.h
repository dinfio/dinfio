...
void init_builtin(int argc, char* argv[]) {
    ...
    DataType* args = core::create_array(0);

    for (int i = 1; i < argc; i++) {
        DataType* argi = new DataType(__TYPE_STRING__);
...
DataType* Connector::__get_value(AST* expression, uint_fast32_t& caller_id) {
...
DataType* Connector::__create_array(uint_fast32_t size) {
    return core::create_array(size);
}

DataType* Connector::__create_object(const string& name) {
    return core::create_empty_object(name);
}

void Connector::__object_set_attribute(DataType* d, string name, DataType* value) {
	d->__value_object->__attributes[name] = value;
}

void Connector::__object_set_function(DataType* d, string name, uint_fast16_t value) {
	d->__value_object->__functions[name] = value;
}

void Connector::__add_constant(const string& name, DataType* value) {
...
void Connector::__remove_garbage(AST* param, DataType* d_param) {
